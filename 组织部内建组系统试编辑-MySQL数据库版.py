...
class Record(Base):
    ...
    operation_time = Column(String(100))
...
def load_user_passwords():
    session = Session()
    users = session.query(UserPassword).all()
    return pd.DataFrame([(u.username, u.password) for u in users], columns=['用户名', '密码'])

# 将数据保存到数据库
def save_data(df):
    session = Session()
    session.query(Record).delete()  # 清空表格
...
def save_user_password(username, password):
    session = Session()
    user_record = UserPassword(id=username, username=username, password=password)
...
# 页面1: 输入界面
def input_page():
    st.markdown("<h2 style='font-size: 24px;'>经组财务报销网站</h2>", unsafe_allow_html=True)
    st.write('当前版本：**20250319**')
    st.write('材料提交说明：每份报销需要提交至少**四份**材料：**发票、支付截图、商品明细、活动人员名单**')
    st.write('①发票：注意发票应开**企业发票而非个人发票**，企业名称为**复旦大学**，税号：**12100000425006117P**。最好选择**商品大类**开发票，其次选择**商品明细**;')
    st.write('②支付截图：指使用支付宝、微信、云闪付等软件支付的凭证，可打开对应软件在**历史账单**中查询。如果是现金支付，则需要拍摄小票或收据上的**支付金额信息**；')
